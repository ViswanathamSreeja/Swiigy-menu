import React from 'react'

export const Header = () => {
    return (
        <div>
            <nav className="navbar navbar-expand-lg navbar-light" style={{ color: 'whitesmoke' }}>
                <a className="navbar-brand" href="#"><img width="100" src="data:image/png;base64," /></a>

                <ul className="navbar-nav">
                    <li className="nav-item active ">
                        <a className="nav-link" href="#">Set location <span className="sr-only">(current)</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#">Click to choose location<i className="bi bi-caret-down"></i> </a>

                    </li>
                </ul>
                <div className="ml-auto" id="navbarNav">
                    <ul className="navbar-nav">

                        <li className="nav-item active">
                            <a className="nav-link mr-5" href="#"><i class="bi bi-eye"></i>Help</a>
                        </li>
                        <li className="nav-item active">
                            <a className="nav-link mr-5" href="#"><i class="bi bi-person-fill"></i>Sign In</a>
                        </li>
                        <li className="nav-item active">
                            <a className="nav-link mr-5" href="#"><i className="bi bi-cart"></i>Cart</a>
                        </li>
                    </ul>
                </div>
            </nav>
            <nav className="navbar navbar-expand-lg navbar-light bg-light py-0" style={{ fontSize: "60%" }}>
                <ul className="navbar-nav">
                    <li className="nav-item ">
                        <a className="nav-link" href="#">Home <span className="ml-1 mr-1">/</span> <span className="sr-only">(current)</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#">Chennai <span className="ml-1 mr-1">/</span> </a>

                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#">Kilpauk <span className="ml-1 mr-1">/</span></a>

                    </li>
                    <li class="nav-item active">
                        <a class="nav-link" href="#">The Belgium Waffle Co.</a>

                    </li>
                </ul>

            </nav>

        </div>
    )
}
export default Header
